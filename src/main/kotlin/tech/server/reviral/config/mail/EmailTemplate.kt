 ...
 */
enum class EmailTemplate(private val title: String, private val subject: String, private val template: String,) {
    SIGNUP("회원가입 인증 안내","[리아비럴] 회원가입 인증번호 안내", "auth"),
    PASSWORD("비밀번호 변경 인증 안내","[리바이럴] 비밀번호 변경 인증코드 안내", "auth"),
    EXCHANGE("포인트 전환 비밀번호 변경 인증 안내","[리바이럴] 포인트 전환 비밀번호 변경 인증코드 안내", "auth");
